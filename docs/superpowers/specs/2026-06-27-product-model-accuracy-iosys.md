...
- **Phase 4 — iPad + Android.** Repeat 1–3 for iosys iPad and Android sections (confirm their URL/carrier structure; Android brands vary).
  - **✅ iPad DONE (2026-06-27):** dedicated parser `ipad-listing.ts` (gen in leading 【第N世代】, part#/A# at title end no-parens, Wi-Fi vs Wi-Fi+Cellular folded into model_name, size インチ, glass token, region suffix J/LL/ZP/TA) + verified `ipad-specs.ts` (39 models). Category-driven `harvest.ts`. Result: 226 ACTIVE iPad SKUs/59 models (device_category=TABLET), partial UNIQUE on TABLET. Connectivity is per-SKU (in model_name) for tablets — unlike carrier which stays per-unit. See PROJECT_STATE Phase 4 bullets.
  - **▶ Android NEXT:** brand-by-brand (Galaxy/Xperia/AQUOS/Pixel/Xiaomi/OPPO/ZTE/arrows/Zenfone/Huawei/Motorola/HTC/BlackBerry). Owner direction: complete iosys lineup for kaitori; cheap imports (Blackview/Realme/etc.) out. Each brand needs its own parser grammar + spec reference.

...
- **Script:** `.claude/hooks/commit-state-reminder.sh`
- **Behavior:** Non-blocking reminder. When a `git commit` has just run, if `docs/PROJECT_STATE.md` is **not** among the staged/modified files, inject a reminder:
  > "Committing changes — check off completed todo(s) in the active plan and update `docs/PROJECT_STATE.md` (Now / Recently shipped / touched-by) as a follow-up commit."

...
optional-code brand via `nameConsumeRe`) → OPPO (next) → arrows → Huawei → Zenfone/ROG → Motorola razr.**
Recipe below. Also fold in the **deferred legacy-Samsung reconcile** (+ now the deferred Xiaomi reconcile, ~33 rows)

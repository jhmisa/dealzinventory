-- Nothing + CMF-by-Nothing fill-gaps promotion (2026-07-01, NEW BRAND).
-- Promotes clean Nothing/CMF SKUs harvested from iosys (iosys_catalog.device_category='ANDROID',
-- brand='Nothing') into product_models, keyed on (brand, model_name, storage, color) per the Android
-- research. ADDITIVE + idempotent (NOT-EXISTS guard). brand normalizes to 'Nothing' via
-- canonical_brands (already present) — the NOT-EXISTS guard still compares lower(brand) defensively.
-- Nothing/CMF are CODE-LESS (JP SIM-free / Rakuten only) so model_number is always NULL. The
-- Phone (3a) Lite cards omit storage in the title → those land storage NULL (flagged, never guessed).
WITH src AS (
  ...
  WHERE device_category = 'ANDROID' AND lower(brand) = 'nothing'

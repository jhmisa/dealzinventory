-- HTC fill-gaps promotion (2026-07-01, NEW BRAND — the last new iosys phone brand in scope).
-- Promotes clean HTC SKUs harvested from iosys (iosys_catalog.device_category='ANDROID', brand='HTC')
-- into product_models, keyed on (brand, model_name, storage, color) per the Android research.
-- ADDITIVE + idempotent (NOT-EXISTS guard, lower(brand) defensive). HTC is a CLOSED JP lineup. The
-- au/SoftBank codes are coarse for U11/10 (collapse to one product_model); the J-series KEEP their
-- code IN model_name (distinct devices). Most SIM-free Desire titles OMIT storage → storage NULL
-- (flagged, never guessed). model_number is the coarse carrier code (NULL for code-less SIM-free).
WITH src AS (
  ...
  WHERE device_category = 'ANDROID' AND lower(brand) = 'htc'

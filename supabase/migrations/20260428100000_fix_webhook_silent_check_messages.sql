-- Fix: webhook_silent alert should also check messages table.
-- Messages can arrive via sync (not just webhooks), so the alert should
-- only fire when NO messages are flowing at all.
...
BEGIN
  ...
  --    Now also checks messages table: if messages are arriving via sync,
  --    the system is healthy even without webhook deliveries.
  IF is_biz_hours THEN
    ...
    ELSE
      -- Auto-resolve if condition cleared (messages flowing via webhook or sync)
